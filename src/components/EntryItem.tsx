...
export function EntryItem({ entry, onDelete, readOnly = false }: { entry: Entry; onDelete: (id: string) => void | Promise<void>; readOnly?: boolean }) {
  ...
  return (
    <li
      ...
      role="listitem"
      aria-label={`${entry.type} entry: ${entry.category}, ${formatRinggit(entry.amount)}${entry.note ? `, ${entry.note}` : ''}`}
    >
      <div className="flex items-start gap-2">
        ...
        <div>
          ...
          <div className="text-xs text-muted-foreground" aria-label={`Time: ${time}`}>{time}</div>
        </div>
      </div>
      ...
      <Button 
        ...
        title={readOnly ? "Cannot delete entries from past days" : "Delete this entry"}
      >
